    ...
    public class DawidSkene
    {
        ...
        public void UpdateAnnotatorErrorRates()
        {
            for (int lid = 0; lid < K; lid++) {
                ...
                for (int j = 0; j < J; j++) {
                    for (int l = 0; l < J; l++) {
                        pi[lid, j, l] = (marginal[j] > 0) ? Math.Round(1.0 * confusion_matrix[j, l] / marginal[j], 5) : 0.0;
        ...
        public void UpdateObjectClassProbabilities()
        {
            for (int oid = 0; oid < I; oid++) {
                ...
                if (correct[oid] != -1) {
                    ...
                    // is unnecessary. The T values should have been
        ...
        public string PrintAnnotatorCostsSummary()
        {
            ...
            for (int k = 0; k < K; k++) {
                ...
                sb.AppendLine(annotator_name + "\t" + Math.Round(100 * cost_naive, 2) + "%\t" + Math.Round(100 * (1 - cost_adj)) + "%\t" +
                    Math.Round(100 * (1 - cost_min)) + "%\t" + contribution);
            }
            return sb.ToString();
        }
        ...
        public static List<Labeling> LoadLabels(string[] lines)
        {
            List<Labeling> labelings = new List<Labeling>();
            foreach (string line in lines) {
                ...
                if (entries.Length == 3) {
                    Labeling lb = new Labeling(entries[0], entries[1], entries[2]);
                    labelings.Add(lb);
                }
            }
            return labelings;
